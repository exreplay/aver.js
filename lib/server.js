...
export default class Server {
    ...
    registerRoutes() {
        ...
        if (fs.existsSync(path.resolve(process.env.API_PATH, './routes'))) {
            const routes = fs.readdirSync(path.resolve(process.env.API_PATH, './routes'));
            forEach(routes, r => {
                const requiredRoute = require(path.resolve(process.env.API_PATH, `./routes/${r}`)).default;
                const route = new requiredRoute();
