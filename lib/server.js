...
export default class Server {
    ...
    setupQueues() {
        if (typeof this.config.queues !== 'undefined' && this.config.queues) global.queue = new Queue();
    }
    ...
    setupSession() {
        if (typeof this.config.session !== 'undefined') this.app.use(new Session(this.config.session));
    }
    ...
    registerRoutes() {
        ...
        if (fs.existsSync(routesPath)) {
            ...
            if(!this.isProd) {
                ...
                watcher.on('ready', () => {
                    console.log("Watching for routes changes on the server");
                    watcher.on('all', () => {
