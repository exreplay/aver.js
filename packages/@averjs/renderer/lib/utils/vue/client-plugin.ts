    ...
    compiler.hooks.make.tap('vue-client-plugin', (compilation) => {
      compilation.hooks.processAssets.tap(
        ...
        () => {
          ...
          const allFiles = uniq(stats.assets as any[]).map((a: { name: string }) => a.name);
          ...
          const asyncFiles = allFiles
            .filter((file: string) => isJS(file) || isCSS(file))
            .filter((file: string) => initialFiles.indexOf(file) < 0);
          ...
          const assetModules = stats.modules.filter((m: { assets: any[] }) => m.assets.length);
          const fileToIndex = (file: string) => manifest.all.indexOf(file);
          stats.modules.forEach((m: AssetModule) => {
            // ignore modules duplicated in multiple chunks
            if (m.chunks.length === 1) {
              const cid = m.chunks[0];
              const chunk = stats.chunks.find(function (c: { id: string }) { return c.id === cid; });
